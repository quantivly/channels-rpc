...
class TestJsonRpcErrorCreation:
    ...
    def test_create_with_required_params(self):
        ...
        error = JsonRpcError(rpc_id=1, code=JsonRpcErrorCode.INVALID_REQUEST)
    ...
    def test_create_with_data(self):
        ...
        error = JsonRpcError(rpc_id=2, code=JsonRpcErrorCode.INVALID_PARAMS, data=data)
    ...
    @pytest.mark.parametrize(
        "rpc_id,code",
        [
            (1, JsonRpcErrorCode.PARSE_ERROR),
            (2, JsonRpcErrorCode.INVALID_REQUEST),
            (3, JsonRpcErrorCode.METHOD_NOT_FOUND),
            (4, JsonRpcErrorCode.INVALID_PARAMS),
            (5, JsonRpcErrorCode.INTERNAL_ERROR),
            (6, JsonRpcErrorCode.GENERIC_APPLICATION_ERROR),
        ],
    )
    ...
    def test_create_with_string_rpc_id(self):
        ...
        error = JsonRpcError(rpc_id="abc123", code=JsonRpcErrorCode.INVALID_REQUEST)

        assert error.rpc_id == "abc123"

    def test_create_with_none_rpc_id(self):
        """Should accept None rpc_id (for notifications)."""
        error = JsonRpcError(rpc_id=None, code=JsonRpcErrorCode.INVALID_REQUEST)
...
class TestJsonRpcErrorSerialization:
    ...
    def test_as_dict_error_structure(self):
        ...
        error = JsonRpcError(rpc_id=1, code=JsonRpcErrorCode.PARSE_ERROR)
        result = error.as_dict()

        assert "code" in result["error"]
        assert result["error"]["code"] == JsonRpcErrorCode.PARSE_ERROR
        assert "message" in result["error"]
        assert result["error"]["message"] == RPC_ERRORS[JsonRpcErrorCode.PARSE_ERROR]

    def test_as_dict_with_data(self):
        """Should include data field in error when provided."""
        data = {"context": "test"}
        error = JsonRpcError(rpc_id=1, code=JsonRpcErrorCode.INTERNAL_ERROR, data=data)
        result = error.as_dict()
    ...
    def test_as_dict_without_data(self):
        ...
        error = JsonRpcError(rpc_id=1, code=JsonRpcErrorCode.INTERNAL_ERROR)
        result = error.as_dict()
...
class TestJsonRpcErrorMessageEnhancement:
    ...
    def test_method_not_found_with_method_name(self):
        """Should include method name in JsonRpcErrorCode.METHOD_NOT_FOUND error."""
        error = JsonRpcError(
            rpc_id=1,
    ...
    def test_method_not_found_without_method_name(self):
        ...
        error = JsonRpcError(
            rpc_id=1, code=JsonRpcErrorCode.METHOD_NOT_FOUND, data={"other": "value"}
        )
        result = error.as_dict()
    ...
    def test_invalid_request_with_version_info(self):
        """Should include version info in JsonRpcErrorCode.INVALID_REQUEST error."""
        error = JsonRpcError(
            rpc_id=1, code=JsonRpcErrorCode.INVALID_REQUEST, data={"version": "1.0"}
        )
        result = error.as_dict()
    ...
    def test_invalid_request_with_various_versions(self, version):
        ...
        error = JsonRpcError(
            rpc_id=1, code=JsonRpcErrorCode.INVALID_REQUEST, data={"version": version}
        )
        result = error.as_dict()
    ...
    def test_invalid_request_with_field_info(self):
        """Should include field info in JsonRpcErrorCode.INVALID_REQUEST error."""
        field_msg = "Missing required field 'method'"
        error = JsonRpcError(
            rpc_id=1, code=JsonRpcErrorCode.INVALID_REQUEST, data={"field": field_msg}
        )
        result = error.as_dict()
    ...
    def test_invalid_params_with_type_info(self):
        """Should include type info in JsonRpcErrorCode.INVALID_PARAMS error."""
        error = JsonRpcError(
            rpc_id=1,
    ...
    def test_no_enhancement_for_generic_error(self):
        """Should not enhance JsonRpcErrorCode.GENERIC_APPLICATION_ERROR message."""
        error = JsonRpcError(
            rpc_id=1,
    ...
    def test_no_enhancement_when_data_not_dict(self):
        ...
        error = JsonRpcError(
            rpc_id=1, code=JsonRpcErrorCode.METHOD_NOT_FOUND, data="string data"
        )
        result = error.as_dict()
...
class TestJsonRpcErrorStringRepresentation:
    ...
    def test_str_matches_as_dict(self):
        ...
        error = JsonRpcError(
            rpc_id=42, code=JsonRpcErrorCode.METHOD_NOT_FOUND, data={"method": "test"}
        )
...
class TestGenerateErrorResponse:
    ...
    def test_generate_with_all_params(self):
        ...
        assert result["jsonrpc"] == "2.0"
        assert result["id"] == 1
        assert result["error"]["code"] == JsonRpcErrorCode.INTERNAL_ERROR
    ...
    def test_generate_without_data(self):
        ...
        result = generate_error_response(
            rpc_id=2, code=JsonRpcErrorCode.PARSE_ERROR, message="Parse failed"
        )
    ...
    def test_generate_with_string_rpc_id(self):
        """Should accept string rpc_id."""
        result = generate_error_response(
            rpc_id="abc", code=JsonRpcErrorCode.INVALID_REQUEST, message="Invalid"
        )
    ...
    def test_generate_with_none_rpc_id(self):
        ...
        result = generate_error_response(
            rpc_id=None, code=JsonRpcErrorCode.INVALID_REQUEST, message="Invalid"
        )
...
class TestErrorCodeConstants:
    ...
    def test_all_codes_in_rpc_errors_dict(self):
        ...
        assert JsonRpcErrorCode.PARSE_ERROR in RPC_ERRORS
        assert JsonRpcErrorCode.INVALID_REQUEST in RPC_ERRORS
        assert JsonRpcErrorCode.METHOD_NOT_FOUND in RPC_ERRORS
        assert JsonRpcErrorCode.INVALID_PARAMS in RPC_ERRORS
        assert JsonRpcErrorCode.INTERNAL_ERROR in RPC_ERRORS
        assert JsonRpcErrorCode.GENERIC_APPLICATION_ERROR in RPC_ERRORS
        assert JsonRpcErrorCode.PARSE_RESULT_ERROR in RPC_ERRORS

    def test_rpc_errors_messages(self):
        """RPC_ERRORS should have appropriate messages."""
        assert RPC_ERRORS[JsonRpcErrorCode.PARSE_ERROR] == "Parse Error"
        assert RPC_ERRORS[JsonRpcErrorCode.INVALID_REQUEST] == "Invalid Request"
        assert RPC_ERRORS[JsonRpcErrorCode.METHOD_NOT_FOUND] == "Method Not Found"
        assert RPC_ERRORS[JsonRpcErrorCode.INVALID_PARAMS] == "Invalid Params"
        assert RPC_ERRORS[JsonRpcErrorCode.INTERNAL_ERROR] == "Internal Error"
        assert (
            RPC_ERRORS[JsonRpcErrorCode.GENERIC_APPLICATION_ERROR]
            == "Application Error"
        )
        assert (
