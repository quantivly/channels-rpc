...
from channels_rpc import JsonRpcError
...
from example.django_example.routing import application
...
class MyTest(aiounittest.AsyncTestCase):
    ...
    async def test_inadequate_request(self):
        ...
        assert response["error"] == {
            "code": JsonRpcConsumerTest.INVALID_REQUEST,
            "message": JsonRpcConsumerTest.errors[JsonRpcConsumerTest.INVALID_REQUEST],
        }
        ...
        await client.send_json_to(
            {"id": "2", "jsonrpc": "2.0", "method": 2, "params": {}}
        )
        response = await client.receive_json_from()
        assert response["error"] == {
            "code": JsonRpcConsumerTest.INVALID_REQUEST,
        ...
        # wrong method name
        await client.send_json_to(
            {"id": "2", "jsonrpc": "2.0", "method": "_test", "params": {}}
        )
        response = await client.receive_json_from()
        assert response["error"] == {
            "code": JsonRpcConsumerTest.METHOD_NOT_FOUND,
            "message": JsonRpcConsumerTest.errors[JsonRpcConsumerTest.METHOD_NOT_FOUND],
        }
        ...
        response = await client.receive_json_from()
        assert response["error"] == {
            "code": JsonRpcConsumerTest.PARSE_ERROR,
            "message": JsonRpcConsumerTest.errors[JsonRpcConsumerTest.PARSE_ERROR],
        }
    ...
    async def test_unexisting_method(self):
        ...
        await client.connect()
        await client.send_json_to(
            {"id": 1, "jsonrpc": "2.0", "method": "unknown_method", "params": {}}
        )
        response = await client.receive_json_from()
        assert response["error"] == {
            "code": JsonRpcConsumerTest.METHOD_NOT_FOUND,
    ...
    async def test_method(self):
        ...
        await client.connect()
        await client.send_json_to(
            {"id": 1, "jsonrpc": "2.0", "method": "ping2", "params": {}}
        )
    ...
    async def test_parsing_with_good_request_wrong_params(self):
        ...
        await client.send_json_to(
            {"id": 1, "jsonrpc": "2.0", "method": "ping2", "params": ["test"]}
        )
        msg = await client.receive_json_from()
        self.assertIn(
            msg["error"]["message"],
            [
                "ping2() takes no arguments (1 given)",  # python 2
                "ping2() takes 0 positional arguments but 1 was given",
            ],
        )  # python 3
        await client.disconnect()
    ...
    async def test_parsing_with_good_request_ainvalid_paramas(self):
        ...
        await client.connect()
        await client.send_json_to(
            {"id": 1, "jsonrpc": "2.0", "method": "ping2", "params": True}
        )
        msg = await client.receive_json_from()
        self.assertEqual(
            msg["error"],
            {
                "code": JsonRpcConsumerTest.INVALID_PARAMS,
                "message": JsonRpcConsumerTest.errors[
    ...
    async def test_parsing_with_good_request(self):
        ...
        await client.connect()
        await client.send_json_to(
            {"id": 1, "jsonrpc": "2.0", "method": "ping", "params": [False]}
        )
        msg = await client.receive_json_from()
    ...
    async def test_id_on_good_request(self):
        ...
        await client.send_json_to(
            {"id": 52, "jsonrpc": "2.0", "method": "ping", "params": {}}
        )
        msg = await client.receive_json_from()
    ...
    async def test_id_on_errored_request(self):
        ...
        await client.send_json_to(
            {"id": 52, "jsonrpc": "2.0", "method": "ping", "params": ["test"]}
        )
        msg = await client.receive_json_from()
        self.assertEqual(msg["id"], 52)
        await client.disconnect()
    ...
    async def test_error_on_rpc_call(self):
        ...
        msg = await client.receive_json_from()
        self.assertEqual(msg["error"]["message"], "pong_with_error")
        await client.disconnect()
    ...
    async def test_error_on_rpc_call_with_data(self):
        ...
        await client.send_json_to(
            {"id": 1, "jsonrpc": "2.0", "method": "ping_with_error_data", "params": {}}
        )
        msg = await client.receive_json_from()
        self.assertEqual(msg["id"], 1)
        self.assertEqual(
            msg["error"]["code"], JsonRpcConsumerTest.GENERIC_APPLICATION_ERROR
        )
    ...
    async def test_namesake_consumers(self):
        # Changed name to TestNamesakeJsonRpcConsumer2 to prevent overlapping with
        # "previous" MyJsonRpcConsumer
        ...
        self.assertEqual(
            Context1.TestNamesakeJsonRpcConsumer2.get_rpc_methods(), ["method1"]
        )
        self.assertEqual(
            Context2.TestNamesakeJsonRpcConsumer2.get_rpc_methods(), ["method2"]
        )
    ...
    async def test_jsonrpcerror_dumping(self):
        import json

        exception = JsonRpcError(
            1, JsonRpcConsumerTest.GENERIC_APPLICATION_ERROR, data=[True, "test"]
        )
        ...
        self.assertEqual(
            json_res["error"]["code"], JsonRpcConsumerTest.GENERIC_APPLICATION_ERROR
        )

    async def test_session_pass_param(self):
        from channels.sessions import SessionMiddlewareStack

        from example.django_example.routing import websocket_urlpatterns
        ...
        await client.send_json_to(
            {"id": 1, "jsonrpc": "2.0", "method": "ping_set_session", "params": {}}
        )
        msg = await client.receive_json_from()
        self.assertEqual(msg["result"], "pong_set_session")
        await client.send_json_to(
            {"id": 1, "jsonrpc": "2.0", "method": "ping_get_session", "params": {}}
        )
        msg = await client.receive_json_from()
    ...
    async def test_custom_json_encoder(self):
        ...
        assert (
            msg["error"]["message"]
            == JsonRpcConsumerTest.errors[JsonRpcConsumerTest.PARSE_RESULT_ERROR]
        )

        await client_django.send_json_to(
            {"id": 1, "jsonrpc": "2.0", "method": "test_method1", "params": {}}
        )
        msg = await client_django.receive_json_from()
        self.assertEqual(msg["result"], {"date": some_date.isoformat()[:-3]})
        await client.disconnect()
        ...
        await client.send_json_to(
            {
                "id": 1,
                "jsonrpc": "2.0",
                "method": "ping_set_session",
                "params": ["name_of_function", "value_of_function"],
            }
        )
        msg = await client.receive_json_from()
        self.assertEqual(
            msg["result"], ["pong_set_session", "value_of_function", "name_of_function"]
        )
        await client.send_json_to(
            {
                "id": 1,
                "jsonrpc": "2.0",
                "method": "ping_get_session",
                "params": {
                    "name2": "name2_of_function",
                    "value2": "value2_of_function",
                },
            }
        )
        msg = await client.receive_json_from()
...
class TestsNotifications(aiounittest.AsyncTestCase):
    async def test_channel_notifications(self):
        ...
        def send_to_reply_channel(**kwargs):
            consumer = kwargs["consumer"]
            consumer.notify_channel("notification.ownnotif", {"payload": 12})
            return True
        ...
        await client.send_json_to(
            {"id": 1, "jsonrpc": "2.0", "method": "send_to_reply_channel", "params": []}
        )
